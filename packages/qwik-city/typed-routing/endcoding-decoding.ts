...
/*
    ...
    the following would go in the user-response.ts file around line ~120:

    const urlObject = new URL(url);
    const searchParams = decodeQueryParams(urlObject.searchParams);
    const pathParams = decodePathParams(params);
    ...
    See notes in the handler() function in helpers.ts for an alternative approach
*/
